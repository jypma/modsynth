...
smoothing = true;
smoothing_radius = 4;				// tweak on this one, how much smoothing to apply
...
pointy_external_indicator_pokey_outey_ness = -2; //
...
sphere_quality_of_indentations = 12;
...
cylinder_number_of_indentations = 12;
cylinder_quality_of_indentations = 50;
radius_of_cylinder_indentations_top = 4.3;
...
if(indentations_cylinder == true)
			{
                start = (pointy_external_indicator) ? 0 : 0;
                end = (pointy_external_indicator) ? cylinder_number_of_indentations - 2 : cylinder_number_of_indentations;
			for (z = [start:end])
				{
